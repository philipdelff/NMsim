...
##'     NMsim, typically called `NMsim_paths.rds`. Can also be a table
...
### recognized formats:
    ...
    if(dt.x[is.res!=TRUE&is.ModTab!=TRUE,.N]>0) stop("Not all objects in `x` recognized by NMreadSim. They should be either (normally) path to `rds` files or (mostly for programming) tables of simulation model information.")
