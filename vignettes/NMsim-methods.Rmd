...
## A temporary note on syntax
The first release of `NMsim` requires methods to specified with three
colons like `method.sim=NMsim:::NMsim_typical`. This will still work
but will not be required in future releases where the shorter
`method.sim=NMsim_typical` will be sufficient.
...
simres.typ <- NMsim(file.mod=file.mod,
                    data=dat.sim,
                    name.sim="typSubj",
                    method.sim=NMsim:::NMsim_typical)
...
all ETA's set to 0 is using `method.sim=NMsim:::NMsim_typical`.
