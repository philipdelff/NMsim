...
simres.varka <- NMsim(file.mod=file.mod,
                     ...
                     ,modify.model=list(PK=add("TVKA=TVKA/KASCALE","KA=KA/KASCALE"))
                      )
...
We can use this notation to modify `TVKA` when it is first defined in
the control stream.
...
simres.varka2 <- NMsim(file.mod=file.mod,
                     ...
                     ,name.sim="varka2"
                     ,seed.R=12345
                     ,modify.model=list(PK=function(x) sub("(.*TVKA *=.*)","\\1/KASCALE",x))
                      )
