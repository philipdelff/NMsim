...
NMsim where to find Nonmem (by settin `path.nonmem`) for this to work.
...
`method.update.inits="psn"` has two advantages over the
NMsim-provided method. It is widely used, and it keeps
...
(`BLOCK`). `method.update.inits="nmsim"` takes a very simple approach. Reading the `OMEGA` matrix  from the `.ext` file, this is
not relying on interpreting Nonmem code at all. The support of systems without `PSN`
would be the main reason one would choose `method.update.inits="nmsim"`
