...
simres <- NMsim(file.mod=files.2.mod,
                data=dat.sim1
                )
```
...
```{r,eval=FALSE}
simres.pi <- simres.n1000.2[,setNames(as.list(quantile(IPRED,probs=c(.05,.5,.95))),cc(ll,median,ul)),
...
res.known <- NMsim(file.mod=file.mod,
                   data=dat.sim1.known,
                   name.sim="known1",
...
```{r,eval=FALSE}
ggplot(as.data.table(res.known)[EVID==2],aes(TIME,IPRED,colour=factor(ID)))+
    geom_line()
```
