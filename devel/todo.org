...
*** TODO [#A] dir.res should not be needed when file.res is provided
*** TODO [#A] Create only one rds per .mod
I think...
*** TODO [#A] Should invisibly return the table of models
*** TODO [#A] NMorderColumns must also be appied if data is a list
