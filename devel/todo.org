...
It used to be done in NMsim. But it has to be a list in each row of the NMsimTab object specifying what to transform. Then do someting like this.
#+begin_src R
...
#+end_source
*** TODO Support changing paths after running NMsim
...
*** TODO [#A] Create only one rds per .mod
I think...
