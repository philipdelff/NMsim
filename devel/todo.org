...
** NMsim
*** TODO Reduce number of needed arguments
...
** plain nonmem run
*** TODO Create dir, Copy files into it
**** Update .mod to use local copy of input data
Or not? Copy input data around so it is available for Nonmem and for reading after.
