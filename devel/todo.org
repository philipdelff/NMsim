...
** NMsim
*** TODO Support method.sim=NULL for doing nothing to the control stream.
*** DONE Support additional args to methods
For now, ellipses used
*** TODO Support submitting jobs to queue system
*** TODO [#A] Go through selection of execution method
*** TODO Define a more flexible structure to return
Currently the sim methods return just files.needed. We need to be able to return 
...
*** TODO Modularize simulation methods
**** DONE Handle seed and subproblems outside modules
**** TODO How to handle replace.sim?
Should that only be available for the default method?
*** TODO Support simulation of new models from covariance step
