...
*** TODO Setting for paths to executables
Path to nonmem, psn, other ways to exec.

*** TODO Fork NMwriteSection from NMdata
** NMsim
*** TODO [#A] Create simulations dir if create.dir=TRUE
If not await user input to create or not. Unless
quiet=T&&create.dir==FALSE in which case to exit with error.
